...
// esta funcion es para Listar y Filtrar
function listarMedicamentos(nombre = "", categoria = "") {
  // se consumen la URL con parámetros de búsqueda
  let url = `${API_URL}?nombre=${nombre}&categoria=${categoria}`;

  $.get(url, function (res) {
    let rows = "";
    res.forEach((m) => {
      // validación para evitar el error de fecha nula
      let fechaParaInput = m.fecha_expiracion
        ? m.fecha_expiracion.split("T")[0]
        : "";
      let fechaMostrar = m.fecha_expiracion
        ? new Date(m.fecha_expiracion).toLocaleDateString()
        : "N/A";
